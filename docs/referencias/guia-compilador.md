...
> Documento original fornecido pelo professor (repositório da disciplina), reproduzido na íntegra como referência.

**Fonte:** `semana 01/docs/Guia - Projeto de um compilador.md` — repositório [sergioaafreitas/COMP1](https://github.com/sergioaafreitas/COMP1)

## Passos para a implementação prática de um compilador
...
- Testar a geração do código com programas-exemplo.
...
- Já começar a pensar em como integrar a geração de código intermediário, pois isso virá na próxima Sprint.
...
**Tarefas e Atividades**
1. Projetar e implementar otimizações simples: remover instruções redundantes, constant folding etc.
...
- Manter versões de backup, pois qualquer problema grave de última hora pode ser mitigado se houver commits estáveis.
...
1. Usar as quartas-feiras para rodar as "dailies" e revisar backlog de tarefas, mantendo transparência sobre o que cada um está fazendo.
2. Planejar reuniões de review no fim de cada Sprint (preferencialmente na própria quarta-feira) para demonstrar o que foi concluído e alinhar o que entra no próximo Sprint.
3. Sempre que possível, manter testes automatizados (ainda que simples) para cada fase: léxica, sintática, semântica, geração de código e execução final.
4. Ficar atento às datas dos formulários (P1 e P2) e às apresentações — qualquer falha na entrega ou no preenchimento derruba a nota daquela avaliação.
5. Considerar a possibilidade de perda de um integrante depois da metade da disciplina: distribuam conhecimento e documentem o máximo possível para não perderem ritmo.
6. Não esquecer de manter folga no cronograma para lidar com imprevistos (bugs complexos, prazos de outras disciplinas, feriados e eventos).

> Esse planejamento é apenas um exemplo — cada equipe pode ajustá-lo conforme seu ritmo, desde que respeitem os marcos (P1, P2, entregas finais e entrevistas).

---

## Adaptação desta equipe

Esta equipe adaptou a sugestão genérica acima às datas oficiais da turma (Grupo 01) e à dupla Mini-C → TypeScript. Ver o resultado em [Cronograma & Marcos](../cronograma.md) e no menu **Sprints**, na barra lateral.

Ver também: [Proposta de Trabalho](proposta-trabalho.md) (requisitos formais e critérios de avaliação) e [Sobre o Projeto](../projeto.md) (aplicação destes passos ao Mini-C/TypeScript desta equipe).
